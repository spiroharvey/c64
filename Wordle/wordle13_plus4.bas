   10 scnclr
   15 x=rnd(-ti)
   16 for t=0 to 15 : read cl
   17 poke 275+t,cl
   18 next t
   19 data 0,113,50,83,52,79,46,103,56,41,66,49,65,111,70,81
   20 color0,1 : color4,1
   25 key1,"{f1}"
   30 a$="UCI"
   31 b$="B B"
   32 c$="JCK"
   ...
   45 dr=peek(66)*256+peek(65)+6
   50 printtab(15)"{yel}w o r d l e         {cyn}v1.3"
   51 print"{lgrn} DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"
   52 print" {pur} c=64 conversion by spiro harvey,2022"
   53 print" {cyn}  (https://youtube.com/c/notawizard)"
   54 print
   56 print"         {pink}original by josh wardle"
   57 print"     {lblu}special thanks to dan sanderson"
   58 print"       {orng}plus/4 porting by luca/fire"
   59 print"{lgrn} DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"
   ...
   70 print" {down}{lgrn}guess the {yel}wordle{lgrn} in 6 tries."
  ...
  110 print"{down} DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"
  ...
  115 goto 112
  120 print"    {lblu}";a$;"{wht}";a$;a$;a$;a$
  125 print"    {lblu}BwB{wht}BeBBaBBrBByB"
  130 print"    {lblu}";c$;"{wht}";c$;c$;c$;c$
  140 print" {lgrn}the letter {lblu}w{lgrn} is in the word and in"
  ...
  165 print" {lgrn}the letter {brn}l{lgrn} is in the word, but at"
  170 print" wrong spot."
  180 gosub 250
  200 print"{wht}    ";a$;a$;a$;"{pink}";a$;"{wht}";a$
  205 print"    {wht}BvBBaBBgB{pink}BuB{wht}BeB          "
  210 print"    {wht}";c$;c$;c$;"{pink}";c$;"{wht}";c$
  215 print" {lgrn}the letter {pink}u{lgrn} is not in the word."
  220 gosub 250
  230 goto 300
  250 rem ------------- screen pause ---
  255 print"        {down}{lred}press {rvon}enter{rvof} to continue...{lgrn}"
  260 getkey k$
  ...
  300 rem -------------- game screen ---
  305 r=int(rnd(1)*nw)
  310 ia=dr+36*int(r/5)+(r-int(r/5)*5)*6
  ...
  350 next
  351 print"{yel}":poke 2048+39,103
  355 poke 2030,64
  400 rem ---------------- main loop ---
  ...
  520 printtab(12);"{cyn}huzzah!! you win!{lgrn}"
  ...
  665 poke 3072+(ln*40)+11,90
  670 poke 2048+(ln*40)+11,66
  675 return
  680 rem -------------- update line ---
  685 poke 3072+(ln*40)+11,32
  690 ln=(at*3)
  695 return
  700 rem ---------------- del char ---
  705 x=c-1
  710 gs$=left$(gs$,x)
  715 poke 3072+(ln*40)+(x*3)+14,32
  720 c=x
  725 goto 610
  750 rem ------------- accept char ---
  755 if c>=5 then goto 610
  760 gs$=gs$+k$
  765 poke 3072+(ln*40)+(c*3)+14,asc(k$)-64
  770 c=c+1
  775 goto 610
  800 rem ---------------- find word ---
  801 rem find gs$ in data quickly
  802 rem f = found:-1 if found, 0 if not
  803 rem expects nw=# of words, dr=addr of first word
  ...
  855 ia=dr+36*int(i/5)+(i-int(i/5)*5)*6
 ...
 1005 for z=1 to 5
 1010 if ga$(z)="+" then cl=111:gosub 1100
 1020 if ga$(z)="-" then cl=41:gosub 1100
 1030 if ga$(z)=""  then cl=49:gosub 1100
 1040 next
 1050 if gs$=pw$ then goto 500
 1060 goto 450 : rem back to loop
 1100 rem ----------- hilight boxes ---
 1105 for c=1 to 3
 1110 poke 2048+((ln-1)*40)+(z*3)+9+c,cl
 1115 next
 1120 for c=1 to 3
 1125 poke 2048+(ln*40)+(z*3)+9+c,cl
 1130 next
 1135 for c=1 to 3
 1140 poke 2048+((ln+1)*40)+(z*3)+9+c,cl
 1145 next
 1150 return
 1200 rem --------------- share mode ---
 1205 for a=1 to 6
 1207 ln=a*3
 1210 for c=1 to 5
 1215 poke 3072+(ln*40)+(c*3)+11,160
 1220 next
 1225 next
 1230 return
 2000 rem ------------- data loader ---
 2010 printtab(13)"{down}{down}{cyn}loading data...";
 2020 c=1
 2030 for w=0 to nw
 2040 read wd$(w)
